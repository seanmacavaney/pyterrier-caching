...
class Hdf5DenseScorerCache(unittest.TestCase):
    def setUp(self):
        if not pt.started():
            pt.init()
    ...
    def test_build_from_iter(self):
        with tempfile.TemporaryDirectory() as d:
            d = Path(d)
            cache = pyterrier_caching.Hdf5DenseScorerCache(d/'cache')
            cache.build([
                {'docno': '1', 'data': 'test'},
    ...
    def test_basic(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            cache = pyterrier_caching.Hdf5DenseScorerCache(d/'cache', scorer_1)
            docnos = Lookup.build(['1', '2', '3', '4', '5'], d/'docnos.npids')
            cache.build(docnos_file=d/'docnos.npids')
            # seeding the cache:
...
class Hdf5SparseScorerCache(unittest.TestCase):
    def setUp(self):
        if not pt.started():
            pt.init()
    ...
    def test_build_from_iter(self):
        with tempfile.TemporaryDirectory() as d:
            d = Path(d)
            cache = pyterrier_caching.Hdf5SparseScorerCache(d/'cache')
            cache.build([
                {'docno': '1', 'data': 'test'},
                {'docno': '2', 'data': 'caching pyterrier'},
                {'docno': '3', 'data': 'fetch me some data'},
    ...
    def test_basic(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            scorer_raises = pt.apply.score(lambda df: 0/0)
            scorer_2 = pt.apply.score(lambda df: 2.)
            cache = pyterrier_caching.Hdf5SparseScorerCache(d/'cache', scorer_1)
            docnos = Lookup.build(['1', '2', '3', '4', '5'], d/'docnos.npids')
            cache.build(docnos_file=d/'docnos.npids')
            # seeding the cache:
            res = cache(pd.DataFrame([
